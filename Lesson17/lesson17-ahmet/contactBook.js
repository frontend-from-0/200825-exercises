...
function findContact(name) {
  ...
  for (let i = 0; i < contacts.length; i++) {
    if (
      contacts[i].name.toLocaleLowerCase().includes(name.toLocaleLowerCase())
    ) {
      console.log(
        `Name: ${contacts[i].name}, Phone: ${contacts[i].phone}, Email: ${contacts[i].email}`
      );
...
//Here my optional sort contact step:
...
function searchContact(name, email, phone) {
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    for (let key in searchObject) {
      if (contacts[i][key] !== searchObject[key]) {
        match = false;
        break;
      }
    }
  ...
  if (count === 0) {
    console.log(
      `No contact found by ${searchBy.join(" and ")} where ${Object.values(
        searchObject
      ).join(" and ")}`
    );
  }
...
sortContactsByName();
displayAllContacts();
// sort step control check are above:
