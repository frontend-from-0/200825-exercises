...
function searchContact(name, phone, email) {
  ...
  searchingLoop: for (let i = 0; i < contacts.length; i++) {

  // Try using for in loop here to loop through searchObject
    const contact = contacts[i]
    for(const personInfo in searchObject){

      const contactData = contact[personInfo];
      const searchingData = searchObject[personInfo];

      if(!String(contactData).includes(searchingData)){
        continue searchingLoop;
