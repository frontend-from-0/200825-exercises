...
function deleteUser(userId) {
  fetch(`https://dummyjson.com/users/${userId}`, {
    ...
    .then(() => {
      cardElement.remove();
      statusContainer.classList.remove('hidden');
      statusContainer.textContent =`${deletedData.firstName} is successfully deleted.`
    ...
    .catch((error) => {
      ...
      statusContainer.classList.remove('hidden');
      statusContainer.textContent = `Failed deleting user ${userId}`;
    });
}
function updateUser(userId){
 
  window.location.href = `./update/index.html?${userId}`
}
fetchUsers()