...
const noStockProducts = productsExercise6.filter((product) => product.quantity > 0);
...
const threeTimesNumbers = numbersExercise7.filter((number) => number % 3 == 0);
console.log(threeTimesNumbers);
...
const shortWords = wordsExercise8.filter((word) => word.split('').length <= 5);
...
const nameWithoutJ = usersExercise10.filter((user) => !user.name.startsWith('J'));
console.log(nameWithoutJ);