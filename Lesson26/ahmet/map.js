...
console.log(
  wordsExercise1.map((word) => word.toUpperCase()),
  wordsExercise1,
);
...
const studendsWithGrade = studentsExercise5.map(
  (student) => `${student.name} ${student.grade}`,
);
console.log(studendsWithGrade);
...
const convertedNumberToString = numbersExercise6.map((number) =>
  number.toString(),
);
...
const splittedSentences = sentencesExercise7.map((sentence) =>
  sentence.split(" "),
);
...
const books = booksExercise9.map((book) => `${book.title} - ${book.author}`);
console.log(books);
// Exercise 10:
...
const currencyPrice = pricesExercise10.map((price) => `$${price}`);
