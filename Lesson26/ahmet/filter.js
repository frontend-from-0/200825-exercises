...
const wordsWithoutA = wordsExercise2.filter(
  (word) => !word.toLowerCase().includes("a"),
);
...
const wordsMoreThan10 = sentencesExercise5.filter(
  (sentence) => sentence.split(" ").length > 10,
);
console.log(wordsMoreThan10);
...
const dividedBy3 = numbersExercise7.filter((number) => number % 3 !== 0);
...
const charLessThan5 = wordsExercise8.filter((word) => word.length <= 5);
console.log(charLessThan5);
...
const studentsHigherC = studentsExercise9.filter(
...
const withOutJ = usersExercise10.filter(
  (user) => !user.name.toLowerCase().startsWith("j"),
);
console.log(withOutJ);
