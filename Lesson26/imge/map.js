...
const studentNames = studentsExercise5.map(student => student.name + " " + student.grade);
...
const stringNumbers = numbersExercise6.map(number => String(number));
...
const wordsArray = sentencesExercise7.map(sentence => sentence.split(" "));
...
const priceWithCurrency = pricesExercise10.map(price => "$" + price);
