...
*/
  ...
  applyDiscount(code){
    ...
    if (discounts.hasOwnProperty(code)){
      console.log(`You earned ${discounts[code]}`)
      const discountPercents = discounts[code];
      const currentTotal = this.#total;
      const discountAmount = (currentTotal * discountPercents) / 100;
      const newTotal = currentTotal - discountAmount;
      console.log(`Congratulations! The new total is ${newTotal}.`)
    }
    else {
      console.log(`${code} doesn't exist. Please check your code.`)
      return this.#total
    }
  }
