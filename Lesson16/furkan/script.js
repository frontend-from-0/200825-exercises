  ...
  for (let i = 0; i < numbers.length; i++) {
    ...
    if (typeof currentValue === 'number') {
      ...
      if (!isNaN(convertedValue)) {
        ...
        console.warn(`Warning: Value at index ${i} is not a valid number and was skipped: ${currentValue}`);
      }
    }
  }
  console.log(sum);
...
// Adjusted the input to showcase the conversion logic for '5'
sumArray([1, 3, '5', 7, -22, 22]); // Output: 16
...
function sumRange(start, end) {
  ...
  console.log(`Sum from ${start} to ${end}: ${sum}`);
}
...
sumObjectValues({ a: 10, b: 20, c: 5, d: 'ignore' }); // 35
...
sumWithDoWhile([10, 20, 30, 40]); // 100
sumWithDoWhile([]); // 0
...
removeDuplicates([1, 2, 'a', 3, 1, 'b', 'a']);
...
filterLongWords(['cat', 'elephant', 'dog', 'mouse'], 4); 
...
logElementsWithIndex(['Red', 'Green', 'Blue']);
...
removeFalsyValues([0, 'hello', false, '', 1, null, undefined, NaN, 'world']); 
...
findWordsWithLetter(['apple', 'banana', 'kiwi', 'Avocado'], 'a'); 
...
function manageQueue(queue, newPerson) {
  ...
  console.log('After Push (new entry):', queue);

  const removedPerson = queue.shift();
  console.log('Shifted (removed) person:', removedPerson);
...
function updateTodoList(todoList, startIndex, deleteCount, ...newTasks) {
  ...
  console.log('After removing tasks:', todoList);

  if (newTasks.length > 0) {
    todoList.push(...newTasks);
  }

  console.log('Final updated list:', todoList);
}
