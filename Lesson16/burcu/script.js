...
function sumObjectValues(obj) {
  ...
  console.log('The sum of object values is:', sum);
}
...
filterLongWords(['apple', 'banana', 'hi', 'ay', 'watermelon', 'you'], 5);
...
logElementsWithIndex(['table', 'banana', 'line', 'javascript', 'cool']);
...
const smallest = findMin([5, 3, 8, 1, 4]);
...
countOccurrences(['apple', 'banana', 'apple', 'kiwi', 'banana', 'apple'], 'apple');
...
function removeFalsyValues(arr) {
  const truthyArray = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i]) { //bu kalıbı gördüğünde aklıma gelmesi gereken tek şey falsy değerleri kontol et olmalıdır. Kısayol gibi tercihe dayalı bir methıd !! //
...
function sumDigits(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    const char = str[i];
    if (!isNaN(char) && char !== ' ') { // bu kısım sayesinde karakterin rakam olup olmadığını anlıyoruz. !=='' bu bölüm sadece boşlukları gidermek için kullanılıyor. //
      sum += Number(char);
    }
  }
  console.log('The sum of digits in the string is:', sum);
}
...
function flattenArray(twoDArray) {
  const flattened = [];
  for (let i = 0; i < twoDArray.length; i++) { //dış döngü
    for (let j = 0; j < twoDArray[i].length; j++) { //iç döngü
...
function findWordsWithLetter(words, letter) {
  ...
  console.log(`Words containing the letter "${letter}":`, result);
}

findWordsWithLetter(['apple', 'banana', 'fig', 'kiwi', 'watermelon'], 'a');
...
pushPopExample(['apple', 'banana', 'kiwi'], 'mango');
...
function manageQueue(queue, newPerson) {
  ...
  console.log('Final queue after shift:', queue);
}
...
updateTodoList(['Study JS', 'Eat breakfast', 'Walk dog'], 1, 1, 'Go shopping', 'Read a book');
